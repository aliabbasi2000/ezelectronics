...
class ProductRoutes {
    ...
    initRoutes() {
        ...
        this.router.post(
            ...
            oneOf([
                body("arrivalDate").isEmpty(),
                body("arrivalDate").isDate().not().isAfter({comparisonDate: Date.now().toString()})
            ]),
        ...
        this.router.patch(
            ...
            body("quantity").isInt({gt: 0}),
            oneOf([
                body("sellingDate").isEmpty(),
                body("sellingDate").isDate().not().isAfter({comparisonDate: Date.now().toString()})
            ]),
            this.authenticator.isLoggedIn,
            this.authenticator.isManager,
            this.errorHandler.validateRequest,
        ...
        this.router.get(
            "/",
            oneOf([
                query("category").isString().isIn(["Smartphone", "Laptop", "Appliance"]).if(query("grouping").isString().equals("category")),
                query("model").isString().notEmpty().if(query("grouping").isString().equals("model")),
                query("model").not().exists().if(query("category").not().exists()).if(query("grouping").not().exists())
            ]),
            this.authenticator.isLoggedIn,
        ...
        this.router.delete(
            ...
            this.authenticator.isAdminOrManager,
            this.errorHandler.validateRequest,
            (_req: any, res: any, next: any) => this.controller.deleteAllProducts()
                .then(() => res.status(200).end())
